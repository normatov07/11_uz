	...
	public function validate($object = NULL, $field_name = NULL)
	{
		...
		foreach ($this->pre_filters as $field => $callbacks)
		{
			foreach ($callbacks as $callback)
			{
				...
				{
/**
 * MYFIX: array_map > arr::map_recursive
 */
				
					foreach ($fields as $f)
					{
						$this[$f] = is_array($this[$f]) ? arr::map_recursive($callback, $this[$f]) : call_user_func($callback, $this[$f]);
					}
				}
				else
				{
					$this[$field] = is_array($this[$field]) ? arr::map_recursive($callback, $this[$field]) : call_user_func($callback, $this[$field]);
				}

/**
 * ENDMYFIX: array_map > arr::map_recursive
 */
			}
		}
