...
class Router_Core {
	...
	public static $controller_path = '/var/www/Projects/11_demo/application/controllers/';
	...
	public static function setup()
	{
		...
		{
			// die(print_r(Kohana::config('routes')));
			// Load routes
			Router::$routes = Kohana::config('routes');
		}
		// die(Router::$current_uri);
		...
		// die(print_r([Router::$rsegments,$paths]));
		foreach (Router::$rsegments as $key => $segment)
		{
			...
			foreach ($paths as $dir)
			{
                ...
                $controller_path = ($controller_path==='cat') ? 'category' : $controller_path;
		...
		{
			// die(print_r($method_segment));
			// Set method
			Router::$method = Router::$rsegments[$method_segment];

			if (isset(Router::$rsegments[$method_segment]))
			{
				// Set arguments
				if(preg_match('/\?/',Router::$rsegments[1])){
					 $dimm =  explode('?',Router::$rsegments[1]);
					 Router::$rsegments[1] = $dimm[0];
					 Router::$rsegments[2]  = $dimm[1];
				}

				// die(print_r(Router::$rsegments));
				if(Router::$rsegments[0] == 'cat'){
					Router::$arguments = array_slice(Router::$rsegments, $method_segment);
				}else{
