<div class="main main2 wlist">
	<div class="lcol">
		<form action="/adm/ro/client/" name="mainForm" id="mainForm" method="post">
			...
			<div class="bluetable bcorns"><i class="ct"><i></i><b></b></i>
				...
				<div class="fr">
					<h5>ИНН:</h5>
					<input type="text" name="inn" maxlength="45" class="dw" value="<?=form::value(@$obj->inn)?>" id="inn" />
				</div>
				<div class="fr">
					<h5>Статус:</h5>
    ...
    <div class="rcol">
		<div class="quicksearch">
			<form action="/adm/ro/client/" method="get" id="quicklistsearch">
				<input type="text" name="q" class="q" value="<?=form::value(@$q)?>" /> <input type="submit" value="Найти" class="submit" /> <span class="busy dn"></span>
			</form>
			<ul class="viewmodes">
				<li><a href="/adm/ro/client/?mode=enabled"<?php if(@$mode == 'enabled') echo ' class="this"' ?>>Активные</a>
				 | <a href="/adm/ro/client/?mode=disabled"<?php if(@$mode == 'disabled') echo ' class="this"' ?>>Неактивные</a></li>
			</ul>			
		</div>
...
<?php	foreach($objList as $item):?>		
			<li<?php if(@$obj->id == $item->id) echo ' class="this"'?>><a name="<?=$item->id?>" href="/adm/ro/client/<?=$item->id?>/"><?=$item->title?></a></li>
<?php 	endforeach;?>
<?php 	echo @$pagination?>
		</ul>
<?php	endif;?>
