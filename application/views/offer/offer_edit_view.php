...
<?php		if($EDITMODE) $category = &$obj->category;
			...
			if(!empty($category)):
				...
				foreach($category->parents as $item):
					...
					echo '<span id="'.$item->id.'"><a class="btn btn-outline-info" href="/offer/'.($EDITMODE?$obj->id.'/edit':'add').$prevURL.'/">'.$item->stitle.'</a></span>';
...
?>
            <div style="margin: 1rem 0">
                <div style="margin-bottom: .5rem"><label class="text-muted">Выберите категорию</label></div>
...
<?php	if($this->isLoggedIn()):?>
		...
		<div class="fr offer_contacts mt-1">
...
<?php		if($this->isAgent() and (!$EDITMODE or @$obj->has_not_user)):?>
				...
				<table id="quick_registration">
					<tbody>
						...
						<tr>
							<th>Имя:</th>
							<td><input type="text" size="24" name="name" id="name" title="Имя" value="<?=form::value(@$obj->name)?>" maxlength="128" class="w" /></td>
						</tr>
					</tbody>
				</table>
...
<?php	endif; // isAgent?>
		<div class="fr">
            ...
            <button class="btn btn-info btn-lg " id="add_form_submit"  type="submit">Добавить объявление</button>
...
<?php else: // EDITMODE ?>
		<div class="fr">
            ...
            <button type="submit" class="btn btn-info btn-lg" id="add_form_submit">Сохранить</button> <span id="ajaxstatus">Отправка данных...</span>
