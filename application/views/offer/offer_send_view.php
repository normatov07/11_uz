<?php if(!empty($offer)):?>
<form id="message_form" method="post" action="<?=$offer->url_send?>">
	...
	<table>
		<tbody>
			<tr>
				...
				<td><?php if($this->isLoggedIn() and $this->user->public_name): echo '<b>'.$this->user->public_name.'</b>'; else: ?><input type="text" name="name" maxlength="64" class="w" value="<?=form::value(@$obj->name)?>" /><?php endif;?></td>
			</tr>
			<tr>
			...
			<tr>
				<th>E-mail получателя: <i class="req">*</i></th>
				<td><input type="text" name="recepient_email" maxlength="128" class="w" value="<?=form::value(@$obj->recepient_email)?>" /></td>
			</tr>
			
			<tr>
				<th>Комментарий:</th>
				<td><textarea name="content" class="w" cols="47" rows="3"><?=form::value(@$obj->content)?></textarea>
